...
/* ── Configuración de seguridad local de autenticación ──
   ✅ FIX: renombrado de SEC → AUTH_CFG para no colisionar con
   window.SEC que security.js define con campos distintos (SESS_KEY, etc.)
   security.js se carga DESPUÉS de auth.js y sobreescribía window.SEC,
   borrando BLOCK_KEY y dejando el bloqueo de login sin funcionar. */
var AUTH_CFG = {
...
function _manejarErrorAuth(error) {
  ...
  var restantes = AUTH_CFG.MAX_INTENTOS - intentosFallidos;
  var msg;
  ...
  switch (error.code) {
    ...
    case 'auth/invalid-credential':
      if (intentosFallidos >= AUTH_CFG.MAX_INTENTOS) {
        _bloquear();
...
function _bloquear() {
  bloqueoHasta = new Date().getTime() + (AUTH_CFG.BLOQUEO_MIN * 60 * 1000);
  /* Persistir para sobrevivir una recarga accidental durante el countdown */
  try { localStorage.setItem(AUTH_CFG.BLOCK_KEY, String(bloqueoHasta)); } catch(e) {}
  iniciarCountdown(bloqueoHasta);
  mostrarError('Acceso bloqueado por ' + AUTH_CFG.BLOQUEO_MIN + ' minutos.');
  document.getElementById('loginPass').value = '';
}

function _restaurarBloqueoSiActivo() {
  try {
    var guardado = parseInt(localStorage.getItem(AUTH_CFG.BLOCK_KEY) || '0', 10);
    if (guardado && guardado > new Date().getTime()) {
      bloqueoHasta = guardado;
      iniciarCountdown(bloqueoHasta);
    } else {
      localStorage.removeItem(AUTH_CFG.BLOCK_KEY);
    }
  } catch(e) {}
}
...
function iniciarCountdown(hasta) {
  ...
  blockInterval = setInterval(function() {
    ...
    if (resto <= 0) {
      ...
      bloqueoHasta     = 0;
      intentosFallidos = 0;
      try { localStorage.removeItem(AUTH_CFG.BLOCK_KEY); } catch(e) {}
